...
@Component({
  ...
  standalone: true,
  imports: [ContainerComponent, HeaderTogglerDirective, SidebarToggleDirective, IconDirective, HeaderNavComponent, NavItemComponent, NavLinkDirective, RouterLink, NgTemplateOutlet, ThemeDirective, DropdownComponent, DropdownToggleDirective, DropdownMenuDirective, DropdownHeaderDirective, DropdownItemDirective, DropdownDividerDirective],
...
export class DefaultHeaderComponent extends HeaderComponent {
  ...
  ngOnInit() {
    ...
    this.emailaddress = storedEmail ? storedEmail.toLowerCase() : '';
    this.profileInitial = this.getInitialFromEmail(this.emailaddress);
  }

  getInitialFromEmail(email: string): string {
    const initial = email.charAt(0).toUpperCase(); // Obtener la inicial en mayúsculas
    return initial;
  }

  toggleDropdown(event: Event): void {
    event.stopPropagation(); // Evitar que el clic se propague
    event.preventDefault();  // Evitar que se active otro comportamiento predeterminado
    this.dropdownOpen = !this.dropdownOpen; // Alternar la visibilidad del dropdown
  }
